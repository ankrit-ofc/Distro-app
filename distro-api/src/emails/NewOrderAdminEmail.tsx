...
export function NewOrderAdminEmail({
  ...
  deliveryAddress,
  paymentMethod,
}: NewOrderAdminEmailProps): React.ReactElement {
  return (
    <Html lang="en">
      <Head />
      <Body style={body}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>🔔 New Order — {orderNumber}</Heading>
            <Text style={subtext}>
              <strong>{storeName}</strong> ({phone}) just placed an order.
            </Text>

            <Row style={tableHeader}>
              <Column style={{ ...th, width: '45%' }}>Item</Column>
              <Column style={{ ...th, width: '15%' }}>Qty</Column>
              <Column style={{ ...th, width: '20%' }}>Price</Column>
            ...
            <Hr style={hr} />
            <Text style={metaLine}>Customer: <strong>{storeName}</strong> — {phone}</Text>
            <Text style={metaLine}>Deliver to: <strong>{deliveryAddress}</strong>, {deliveryDistrict}</Text>
            <Text style={metaLine}>Payment: <strong>{paymentMethod}</strong></Text>

            <Button style={btn} href={`https://distronepal.com/admin/orders`}>
              View in Dashboard
            </Button>
          </Section>
...
const header:      React.CSSProperties = { backgroundColor: '#0D1120', padding: '20px 32px', borderRadius: '8px 8px 0 0' };
...
const btn:         React.CSSProperties = { backgroundColor: '#1A4BDB', color: '#ffffff', padding: '12px 28px', borderRadius: '6px', fontWeight: 'bold', fontSize: '15px', textDecoration: 'none', display: 'inline-block', marginTop: '16px' };
